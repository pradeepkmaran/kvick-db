...
CMD ["sh", "-c", "./build/src/kvick --port $PORT --id ${NODE_ID:-$(hostname)} --grpc 0.0.0.0:$GRPC_PORT --advertise ${ADVERTISE_ADDRESS:-$(hostname):$GRPC_PORT} --raft-port $RAFT_PORT --data-dir $DATA_DIR $( [ -n \"$SEED_NODES\" ] && echo \"--seed $SEED_NODES\" )"]